...
//wuzhu start ²Î¿¼Creature::LoadGoods()
void Item::LoadVipGoods(uint32 menu)
{
	if(lastvipmenu==menu)
		return;
	m_vip_items.clear();

	QueryResult *result = WorldDatabase.PQuery("SELECT item, maxcount, incrtime, ExtendedCost FROM item_vendor WHERE entry = '%u'", menu);
	...
	do
	{
		...
		if (GetVipItemCount() >= MAX_VENDOR_ITEMS)
		{
			sLog.outErrorDb( "Item_Vendor %u has too many items (%u >= %i). Check the DB!", GetEntry(), GetVipItemCount(), MAX_VENDOR_ITEMS );
			break;
		}
		...
		{
			sLog.outErrorDb("Item_Vendor %u have in item list non-existed item %u",GetEntry(),item_id);
			continue;
		}

		uint32 ExtendedCost = fields[3].GetUInt32();
		if(ExtendedCost && !sItemExtendedCostStore.LookupEntry(ExtendedCost))
			sLog.outErrorDb("Item (Entry: %u) has wrong ExtendedCost (%u) for Item_Vendor (%u)",item_id,ExtendedCost,GetEntry());

		AddVipItem( item_id, fields[1].GetUInt32(), fields[2].GetUInt32(), ExtendedCost);
	}
