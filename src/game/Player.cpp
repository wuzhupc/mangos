...
Player::Player (WorldSession *session): Unit(), m_mover(this), m_camera(this), m_achievementMgr(this), m_reputationMgr(this)
{
    ...
    m_vip_integral=0;
	//读取VIP等级
	QueryResult *resultld = LoginDatabase.PQuery("SELECT vip FROM account WHERE id = '%u'", GetSession()->GetAccountId());
	if(resultld)
	{
		Field* fields2 = resultld->Fetch();
		m_Vip = fields2[0].GetUInt32();
		delete resultld;
	}	
	else
	{
		m_Vip=0;
		DETAIL_LOG( "WUZHU Read VIP Error:(unknown account %u).",GetSession()->GetPlayer());
	}
	//wuzhu end
}
...
bool Player::Create( uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId */)
{
    ...
    //wuzhu 设置开始金钱数和积分数
	SetVIP_Integral(sWorld.getWUZHUConfig(WUZHU_Create_Integral));
	//wuzhu end
...
void Player::Update( uint32 update_diff, uint32 p_time )
{
    ...
    if (m_nextSave > 0)
    {
        ...
        {
			//wuzhu start 统计、增加在线积分　输出信息
			uint32 In_VIP_Integral=sWorld.getWUZHUConfig(WUZHU_Integral);//积分增量
			if(m_Vip==5)
				In_VIP_Integral=sWorld.getWUZHUConfig(WUZHU_VIP5_Integral);
			else
				if(m_Vip)
					In_VIP_Integral=In_VIP_Integral*sWorld.getWUZHUConfig(WUZHU_VIP_Integral);
			ModifyVIP_Integral(In_VIP_Integral);//增加300点积分

			GetSession()->SendAreaTriggerMessage(GetWUZHUString(-6000317,this),In_VIP_Integral,GetVIP_Integral());// "澧炲姞%u鐐圭Н鍒嗙幇鍏辨湁%u鐐圭Н鍒.",In_VIP_Integral,GetVIP_Integral());			
			//wuzhu end
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
    ...
    if (int32(getLevel()) >= startLevel)
    {
        ...
        {
            int32 delta = (int32(getLevel()) - startLevel + 1)*MINUTE/10; //wuzhu 修改死亡时虚弱冷却时间
    ...
    if (!(currFields & val))
    {
        ...
        else if (p->area_level > 0)
        {
            ...
            else
            {
                ...
                //wuzhu start 修改获得新区域的经验
				if(m_Vip==5)
				{
					XP=uint32(XP*sWorld.getWUZHUConfig(WUZHU_VIP5_XP_Explore)/sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE));
				}
				else
				if(m_Vip>0)
				{
					XP=uint32(XP*sWorld.getWUZHUConfig(WUZHU_VIP_XP_Explore));
				}
				//wuzhu end
...
int32 Player::CalculateReputationGain(ReputationSource source, int32 rep, int32 faction, uint32 creatureOrQuestLevel, bool noAuraBonus)
{
	...
	if(rep>0)
	{
		rep=int32(sWorld.getConfig(CONFIG_FLOAT_RATE_REPUTATION_GAIN)*rep*percent/100.0f);
		int32 maxrep=int32(sWorld.getWUZHUConfig(WUZHU_MaxReputation));
		if(rep>maxrep)
			return maxrep;
		else
			return rep;
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
    ...
    if(mapid == 530||mapid==0||mapid==1||this->isGameMaster())// Outland //wuzhu 修改可飞行状态
    {
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, Object* questGiver, bool announce)
{
	...
	//wuzhu start 修改获得任务的经验
	uint32 xp = 0;//
	float vipxp=1.0f;
	if(m_Vip==5)
	{
		xp = uint32(pQuest->XPValue( this )*sWorld.getWUZHUConfig(WUZHU_VIP5_XP_Quest));
	}
	else
	{
		if(m_Vip&&sWorld.getWUZHUConfig(WUZHU_VIP_XP_Level)>=getLevel())
		{
			if(getClass()==CLASS_DEATH_KNIGHT&&sWorld.getWUZHUConfig( WUZHU_Hero_NOVipXP)!=0.0f)
				vipxp=1.0f;
			else
				vipxp=sWorld.getWUZHUConfig(WUZHU_VIP_XP_Quest);
		}
    xp = uint32(pQuest->XPValue( this )*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_QUEST)*vipxp);
	}
	if(xp>sWorld.getWUZHUConfig(WUZHU_MaxXP))
		xp=sWorld.getWUZHUConfig(WUZHU_MaxXP);
	//sLog.outError("wuzhu XP:%u#m_vip:%u#VIP5_XP:%f#vipxp:%f#q_status.m_rewarded:%d #pQuest->XPValue( this ):%d",
		//XP,m_Vip,sWorld.getWUZHUConfig(WUZHU_VIP5_XP_Quest),vipxp,q_status.m_rewarded,pQuest->XPValue( this ));                
	//wuzhu end
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
	...
	//wuzhu start
    m_vip_integral=fields[66].GetUInt32();//wuzhu
...
void Player::_LoadTalents(QueryResult *result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if ((getClassMask() & talentTabInfo->ClassMask) == 0&&talent_id!=1867)//wuzhu 修改装备2把双手武器天赋
            {
...
void Player::SaveToDB()
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "INSERT INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, vip_integral) " //wuzhu
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, "
        "?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, "
	...
	//wuzhu start 记录VIP积分
	uberInsert.addUInt32(m_vip_integral);
...
void Player::InitDisplayIds()
{
    ...
    SetObjectScale(DEFAULT_OBJECT_SCALE*sWorld.getWUZHUConfig(WUZHU_SCALE_X));//wuzhu 角色大小倍数
...
//wuzhu start 宝石商店 参考Player::BuyItemFromVendor
bool Player::BuyItemFromItemVendorSlot(ObjectGuid vendorGuid, uint32 vendorslot, uint32 item, uint8 count, uint64 bag, uint8 slot, uint32 menu)
{
	// cheating attempt
	if(count < 1) count = 1;
	...
	Item *p_pItem = GetItemByGuid(vendorGuid);
	//Creature *pCreature = ObjectAccessor::GetNPCIfCanInteractWith(*this, vendorguid,UNIT_NPC_FLAG_VENDOR);
	if (!p_pItem)
	{
		DEBUG_LOG( "WORLD: BuyItemFromItemVendorSlot - Unit (GUID: %u) not found or you can't interact with it.",  vendorGuid.GetString().c_str());
		SendBuyError( BUY_ERR_DISTANCE_TOO_FAR, NULL, item, 0);
		return false;
	}

	// load vendor items if not yet
	p_pItem->LoadVipGoods(menu);

	VipItem* crItem = p_pItem->FindVipItem(item);
	if(!crItem)
	{
		SendBuyError( BUY_ERR_CANT_FIND_ITEM, NULL, item, 0);
		return false;
	...
	{
		if(crItem->count < pProto->BuyCount * count )
		{
	...
	if(price==0)
		price = pProto->ItemLevel*200* count;//乘以物品等级
	if(price==0)
	{
		GetSession()->SendAreaTriggerMessage(GetWUZHUString(-6000315,this));//"姝ょ墿鍝佸凡缁忚闄愬埗鍞嚭锛屽鏈夌枒闂鍜ㄨWUZHU,QQ:85267512");//此物品已经被限制售出，如有疑问请咨询WUZHU,QQ:85267512
		return false;
	}

	//if( GetMoney() < price )
	if(GetVIP_Integral()<price)
	{
		GetSession()->SendAreaTriggerMessage(GetWUZHUString(-6000316,this),price);// " 浣犵殑绉垎涓嶅锛屾鐗╁搧闇�瑕佺Н鍒嗭細%u.",price);//你的积分不够，此物品需要积分：%u
		return false;
	}
	...
	{
		ItemPosCountVec dest;
		InventoryResult msg = CanStoreNewItem( bag, slot, dest, item, pProto->BuyCount * count );
		if( msg != EQUIP_ERR_OK )
		{
			SendEquipError( msg, NULL, NULL ,item);
			return false;
		...
		ModifyVIP_Integral(-(int32)price );
		//GetSession()->SendAreaTriggerMessage("鐜拌繕鍓╃Н鍒嗭細%u",GetVIP_Integral());//现还有积分：%u
		//GetSession()->SendAreaTriggerMessage("浣犳秷璐圭Н鍒嗭細%u",price);//你消费积分：%u
		GetSession()->SendAreaTriggerMessage(GetWUZHUString(-6000318,this),price,GetVIP_Integral());
		...
		if(Item *it = StoreNewItem( dest, item, true ))
		{
			...
			data << uint64(p_pItem->GetObjectGuid());
            data << uint32(vendorslot+1);                   // numbered from 1 at client
            data << uint32(crItem->maxcount > 0 ? count : 0xFFFFFFFF);
            data << uint32(count);
			GetSession()->SendPacket(&data);
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorGuid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (uint32 extendedCostId = crItem->ExtendedCost)
    {
        ...
        if (GetMaxPersonalArenaRatingRequirement(iece->reqarenaslot) < iece->reqpersonalarenarating)
        {
            ...
            //return false;//wuzhu 暂时取消对竞技场的要求
        }
    }
...
void Player::UpdatePotionCooldown(Spell* spell)
{
    ...
    if(!m_lastPotionId  )//wuzhu 修改在战斗时也可以冷却|| isInCombat())
        return;
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    if (!PvP)
    {
		...
		//wuzhu start 根据修改竞技点数和荣誉点
			if(pVictim)
			{
				int clevel=pVictim->getLevel()-getLevel();
				switch(((Creature *)pVictim)->GetCreatureInfo()->rank)
				{
				...
				case CREATURE_ELITE_RAREELITE:
					if(clevel>=-3)
					{
						ModifyHonorPoints((clevel+2)*10+10);//0 5 10 15...
						ModifyArenaPoints(clevel+4);    //1 2 3 4...
					}
					break;
				case CREATURE_ELITE_WORLDBOSS:
					if(clevel>=-3)
					{
						ModifyHonorPoints((clevel+2)*20+20);//0 10 20 30...
						ModifyArenaPoints((clevel+3)*3+3);//3 6 9 12...
					}
					break;
				default:
					if(clevel>=-3)
					{
						ModifyHonorPoints((clevel+2)*4+4);//0 2 4 6...
						ModifyArenaPoints((int)((clevel+3)/2)+1);//1 1 2 2 3 3
					}
				break;
				}
		}
...
//wuzhu start几个自定义绑定位置方法
QueryResult * Player::WUZHU_GetBind()
{
	 return CharacterDatabase.PQuery("SELECT id, guid, position_x, position_y, position_z, map, Zone, maptext FROM wuzhu_bind where guid=%u order by id",GetGUIDLow());
}

bool Player::WUZHU_DelBind(uint32 index)
{
	//error_log("WUZHU:delete from wuzhu_bind where id=%u and guid='%u'",index,GetGUIDLow());
	return CharacterDatabase.PExecute("delete from wuzhu_bind where id=%u and guid='%u'",index,GetGUIDLow());
}
bool  Player::WUZHU_InsertBind(uint32 index)
{
	CellPair cell_val = MaNGOS::ComputeCellPair(GetPositionX(), GetPositionY());
	Cell cell(cell_val);
	...
	//error_log("WUZHU:INSERT INTO wuzhu_bind (id,guid,map,zone,position_x,position_y,position_z,maptext) VALUES ('%u','%u', '%u', '%u', '%f', '%f', '%f','%s-%s-%s')",
		 //  index,GetGUIDLow(), GetMapId(), zone_id, GetPositionX(), GetPositionY(), GetPositionZ(),
		   //(mapEntry ? mapEntry->name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ),
//		   (zoneEntry ? zoneEntry->area_name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ),
		//   (areaEntry ? areaEntry->area_name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ));
...
//GroundZ: %f FloorZ: %f Have height data (Map: %u VMap: %u)
	...
	return CharacterDatabase.PExecute("INSERT INTO wuzhu_bind (id,guid,map,zone,position_x,position_y,position_z,maptext) VALUES ('%u','%u', '%u', '%u', '%f', '%f', '%f','%s-%s-%s')",
		index,GetGUIDLow(), GetMapId(), zone_id, GetPositionX(), GetPositionY(), GetPositionZ(),
		(mapEntry ? mapEntry->name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ),
		(zoneEntry ? zoneEntry->area_name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ),
		(areaEntry ? areaEntry->area_name[GetSession()->GetSessionDbcLocale()] : "<unknown>" ));
}
QueryResult * Player::WUZHU_GetOneBind(uint32 index)
{
	return CharacterDatabase.PQuery("SELECT id, guid, position_x, position_y, position_z, map, Zone, maptext FROM wuzhu_bind where guid=%u and id=%u",GetGUIDLow(),index);
}

bool Player::WUZHU_OpenGuildBank()
{
	if (uint32 GuildId = this->GetGuildId())
	{
		if(Guild* pGuild = sGuildMgr.GetGuildById(GuildId))
		{
			uint32 id=uint32(sWorld.getWUZHUConfig(WUZHU_GUILD_ID));//公会银行ObjectID
			if(id=0)
				return false;

			GameObject* pGameObj = this->SummonGameobject(id,GetPositionX(),GetPositionY(),GetPositionY(),GetOrientation(),300000);
	...
	this->GetSession()->SendGuildCommandResult(GUILD_UNK1, "", ERR_GUILD_PLAYER_NOT_IN_GUILD);
	return false;
}

void Player::WUZHU_SendPointOfInterest(float X, float Y,char const * locName)
{
	PlayerTalkClass->SendPointOfInterest(X,Y,7,99,0,locName);
}

void Player::WUZHU_CompletedAchievement(uint32 achId)
{
	...
	AchievementMgr& mgr = this->GetAchievementMgr();
...
float Player::WUZHU_GetHealthRate()
{
	//if(GetMap()->IsDungeon())
	if(GetMap()->GetMaxPlayers()==5)
		return sWorld.getWUZHUConfig(WUZHU_Health_Dungeon);
	else if(GetMap()->GetMaxPlayers()==10)
		return sWorld.getWUZHUConfig(WUZHU_Health_Raid_10Man);
	else if(GetMap()->GetMaxPlayers()==25)
		return sWorld.getWUZHUConfig(WUZHU_Health_Raid_25Man);
	//if(GetMap()->IsRaid())
	//{
	//	if(GetMap()->GetMaxPlayers()>10)
	//		return sWorld.getWUZHUConfig(WUZHU_Damage_Raid_25Man);
