...
enum WUZHUConfig
{
	WUZHU_VIP5_XP_Kill=0,//VIP5的杀怪经验倍数
	WUZHU_VIP5_XP_Quest,//VIP5的任务经验倍数
	WUZHU_VIP5_XP_Explore,//VIP5的开地图经验倍数
	WUZHU_VIP5_Integral,//VIP5的积分
	WUZHU_Integral,//普通用户积分
	WUZHU_VIP_Integral,//VIP的杀怪经验倍数(乘以默认的经验倍数)
	WUZHU_VIP_XP_Kill,//VIP5的杀怪经验倍数(乘以默认的经验倍数)
	WUZHU_VIP_XP_Quest,//VIP5的杀怪经验倍数(乘以默认的经验倍数)
	WUZHU_VIP_XP_Explore,//VIP5的杀怪经验倍数(乘以默认的经验倍数)
	...
	WUZHU_VIP_MaxLevel,//VIP的最大级别
	WUZHU_MaxXP,//能获取的最多经验
	WUZHU_MaxReputation,//能获取最多声望
	WUZHU_VIP_XP_Level,//VIP经验最高有效等级
	WUZHU_Hero_NOVipXP,//英雄职业无VIP经验
	WUZHU_LOG_VIP5GMLOG,//记录VIP5GM记录，默认0不记录
	WUZHU_CHEAT_SWITCH,//开启假人 默认0
...
class World
{
    public:
        ...
        //wuzhu 配置文件　start
		void setWUZHUConfig(WUZHUConfig wuzhuconfig,float value) { wuzhuconfig_values[wuzhuconfig]=value; }
		float getWUZHUConfig(WUZHUConfig wuzhuconfig) const { return wuzhuconfig_values[wuzhuconfig]; }
		//wuzhu end
    ...
    private:
        ...
        float wuzhuconfig_values[MAX_WUZHUConfig];//wuzhu
