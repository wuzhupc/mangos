...
void World::LoadConfigSettings(bool reload)
{
    ...
    //wuzhu 配置文件读取　start
	wuzhuconfig_values[WUZHU_VIP5_XP_Kill]     = sConfig.GetFloatDefault("WUZHU.VIP5.XP.Kill", 2.0f);
	wuzhuconfig_values[WUZHU_VIP5_XP_Quest]     = sConfig.GetFloatDefault("WUZHU.VIP5.XP.Quest", 2.0f);
	wuzhuconfig_values[WUZHU_VIP5_XP_Explore]     = sConfig.GetFloatDefault("WUZHU.VIP5.XP.Explore", 10.0f);
	wuzhuconfig_values[WUZHU_VIP5_Integral]     = sConfig.GetFloatDefault("WUZHU.VIP5.Integral", 1000.0f);
	wuzhuconfig_values[WUZHU_VIP_XP_Quest]     = sConfig.GetFloatDefault("WUZHU.VIP.XP.Quest", 1.0f);
	wuzhuconfig_values[WUZHU_VIP_XP_Explore]    = sConfig.GetFloatDefault("WUZHU.VIP.XP.Explore", 1.0f);
	wuzhuconfig_values[WUZHU_VIP_XP_Kill]       =sConfig.GetFloatDefault("WUZHU.VIP.XP.Kill", 1.0f);
	...
	wuzhuconfig_values[WUZHU_Pet_TMP_Damage]	=sConfig.GetFloatDefault("WUZHU.Pet.TMP.Damage",1.0f);//宠物法术伤害倍数
	wuzhuconfig_values[WUZHU_Pet_PDA_Damage]	=sConfig.GetFloatDefault("WUZHU.Pet.PDA.Damage",1.0f);//宠物物理伤害倍数
	wuzhuconfig_values[WUZHU_Pet_Scale]	=sConfig.GetFloatDefault("WUZHU.Pet.Scale",1.0f);//宠物倍数
	wuzhuconfig_values[WUZHU_Hero_NOVipXP] = sConfig.GetIntDefault("WUZHU.Hero.NoVipXP", 1);//英雄职来是否无VIP经验设置
	wuzhuconfig_values[WUZHU_VIP_MaxLevel] = sConfig.GetIntDefault("WUZHU.VIP.MaxLevel", 80);//VIP最大等级
	if(wuzhuconfig_values[WUZHU_VIP_MaxLevel] > MAX_LEVEL)
	{
		sLog.outError("WUZHU.VIP.MaxLevel (%i) must be in range 1..100. Set to 80.",wuzhuconfig_values[WUZHU_VIP_MaxLevel]);
		wuzhuconfig_values[WUZHU_VIP_MaxLevel] = MAX_LEVEL;
	}
	wuzhuconfig_values[WUZHU_MaxXP] = sConfig.GetIntDefault("WUZHU.MaxXP", 200000);//最多经验
	if(wuzhuconfig_values[WUZHU_MaxXP] > 5000000)
	{
		sLog.outError("WUZHU.MaxXP (%i) must be in range 0..5000000. Set to 5000000.",wuzhuconfig_values[WUZHU_MaxXP]);
		wuzhuconfig_values[WUZHU_MaxXP] = 5000000;
	}
	wuzhuconfig_values[WUZHU_MaxReputation] = sConfig.GetIntDefault("WUZHU.MaxReputation", 200000);//最多经验
	if(wuzhuconfig_values[WUZHU_MaxReputation] > 500000)
	{
		sLog.outError("WUZHU.MaxXP (%i) must be in range 0..500000. Set to 500000.",wuzhuconfig_values[WUZHU_MaxReputation]);
		wuzhuconfig_values[WUZHU_MaxReputation] = 500000;
	}
	wuzhuconfig_values[WUZHU_VIP_XP_Level] = sConfig.GetIntDefault("WUZHU.VIP.XP.Level", 80);//VIP最高加成等级
	if(wuzhuconfig_values[WUZHU_VIP_XP_Level] > MAX_LEVEL)
	{
		sLog.outError("WUZHU.VIP.XP.Level (%i) must be in range 1..100. Set to 80.",wuzhuconfig_values[WUZHU_VIP_MaxLevel]);
		wuzhuconfig_values[WUZHU_VIP_XP_Level] = MAX_LEVEL;
	}
	wuzhuconfig_values[WUZHU_LOG_VIP5GMLOG]	=sConfig.GetFloatDefault("WUZHU.LOG.VIP5GMLOG",0.0f);//
	wuzhuconfig_values[WUZHU_CHEAT_SWITCH]	=sConfig.GetIntDefault("WUZHU.CHEAT.SWITCH", 0);// 
	wuzhuconfig_values[WUZHU_CHEAT_COUNT]	=sConfig.GetIntDefault("WUZHU.CHEAT.COUNT", 49);// 
	wuzhuconfig_values[WUZHU_MOB_MAXHEA]=sConfig.GetIntDefault("WUZHU.MOB.MAXHEA", 50000000);
	wuzhuconfig_values[WUZHU_MOB_HEAADD]=sConfig.GetIntDefault("WUZHU.MOB.HEAADD", 100);
	if(wuzhuconfig_values[WUZHU_MOB_HEAADD] < 0)
	{
		sLog.outError("WUZHU.MOB.HEAADD (%i) must be in range 0..100. Set to 100.",wuzhuconfig_values[WUZHU_MOB_HEAADD]);
		wuzhuconfig_values[WUZHU_MOB_HEAADD] = 100.0f;
	}
	wuzhuconfig_values[WUHZU_MoveSpeed]=sConfig.GetFloatDefault("WUZHU.MoveSpeed", 1.0f);
	if(wuzhuconfig_values[WUHZU_MoveSpeed] <=0.0f||wuzhuconfig_values[WUHZU_MoveSpeed] >100.0f)
	{
		sLog.outError("WUZHU.MoveSpeed (%f) must be in >0 and <=100. Set to 1.0f.",wuzhuconfig_values[WUHZU_MoveSpeed]);
		wuzhuconfig_values[WUHZU_MoveSpeed] = 1.0f;
	}
	wuzhuconfig_values[WUZHU_SCALE_X]=sConfig.GetFloatDefault("WUZHU.SCALE_X", 1.0f);
	if(wuzhuconfig_values[WUZHU_SCALE_X] <=0.0f||wuzhuconfig_values[WUZHU_SCALE_X] >100.0f)
	{
		sLog.outError("WUZHU.SCALE_X (%f) must be in >0 and <=100. Set to 1.0f.",wuzhuconfig_values[WUZHU_SCALE_X]);
