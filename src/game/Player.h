...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
        ...
        bool BuyItemFromItemVendorSlot(ObjectGuid vendorGuid, uint32 vendorslot, uint32 item, uint8 count, uint64 bag, uint8 slot, uint32 menu); //wuzhu 宝石商店
        ...
        //wuzhu start vip_integral
		void SetVIP(uint32 value)
		{
			m_Vip=value;				
		}
		uint32 GetVIP(){return m_Vip;}
		uint32 GetVIP_Integral() { return m_vip_integral; }
		void ModifyVIP_Integral( int32 d )
		{
			if(d < 0)
				SetVIP_Integral(m_vip_integral > uint32(-d) ? m_vip_integral + d : 0);
			else
				SetVIP_Integral (m_vip_integral < uint32(MAX_MONEY_AMOUNT - d) ? m_vip_integral + d : MAX_MONEY_AMOUNT);
		}
		void SetVIP_Integral( uint32 value ){m_vip_integral=value;}
		QueryResult * WUZHU_GetBind();
		QueryResult * WUZHU_GetOneBind(uint32 index);
		bool WUZHU_DelBind(uint32 index);
		bool WUZHU_InsertBind(uint32 index);
