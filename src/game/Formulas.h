...
namespace MaNGOS
{
    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
			...
			float vipxp=1.0f;
			if(pl->GetVIP()==5)
				return (uint32)(xp_gain*sWorld.getWUZHUConfig(WUZHU_VIP5_XP_Kill));
			else
			if(pl->GetVIP()>0&&sWorld.getWUZHUConfig(WUZHU_VIP_XP_Level)>=pl->getLevel())
			{
				if(pl->getClass()==CLASS_DEATH_KNIGHT&&sWorld.getWUZHUConfig( WUZHU_Hero_NOVipXP)!=0.0f)
					vipxp=1.0f;
				else
					vipxp=sWorld.getWUZHUConfig(WUZHU_VIP_XP_Kill);
			}
			xp_gain=xp_gain*vipxp;
			//wuzhu end
