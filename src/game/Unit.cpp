...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, DamageInfo* damageInfo, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
	...
	{
		if(damage>(uint32)sWorld.getWUZHUConfig(WUZHU_Max_Deal))
		{

			error_log("DealDamage this:%d,pvictim:%d,damage:%d",this->GetEntry(),pVictim->GetEntry(),damage);
			damage=(uint32)sWorld.getWUZHUConfig(WUZHU_Max_Deal);
		}
	}
	//wuzhu end
...
void Unit::CastSpell(Unit* Victim, SpellEntry const *spellInfo, bool triggered, Item *castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
	...
	//wuzhu start 略过的魔法列表 100903
	const uint32 filterspellidlist[]={26615,26662,27680,28498,35595,45078,46587,47008,55816,61632,
		61714,61715,62555,64112,64238,65126,68378,72525};
	int length=sizeof(filterspellidlist)/sizeof(int);
	for(int i=0;i<length&&spellInfo->Id<=filterspellidlist[i];i++)
	if (spellInfo->Id==filterspellidlist[i])
	{
		sLog.outError("WUZHU CastSpell: %d spell by caster: %s if filter!!!!",filterspellidlist[i],GetObjectGuid().GetString().c_str());
		return;
	}
	//wuzhu end
...
void Unit::CalculateSpellDamage(DamageInfo *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, float DamageMultiplier)
{
	...
	if(GetTypeId()==TYPEID_PLAYER)
	{
		...
		//sLog.outError("WUZHU CalculateSpellDamage damage start:%d",damage);
			damage =uint32(damage*dxp);
		//sLog.outError("WUZHU CalculateSpellDamage damage end:%d",damage);
	}
	else
