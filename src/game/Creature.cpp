...
void Creature::SelectLevel(const CreatureInfo *cinfo, float percentHealth, float percentMana)
{
    ...
    uint32 health = uint32(healthmod * (minhealth + uint32(rellevel*(maxhealth - minhealth))));
		//wuzhu start
	if(health>(uint32)sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA)&&sWorld.getWUZHUConfig(WUZHU_MOB_HEAADD)<100.0f)
	{
		health=uint32(sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA)+(health-sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA))*sWorld.getWUZHUConfig(WUZHU_MOB_HEAADD)/100);
		//sLog.outError("wuzhu head %u,WUZHU_MOB_MAXHEA %f,WUZHU_MOB_HEAADD %f",health,sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA),WUZHU_MOB_HEAADD);
	}
	//wuzhu end
...
bool Creature::LoadFromDB(uint32 guidlow, Map *map)
{
    ...
    if (curhealth)
    {
		...
		//wuzhu start
		if(curhealth>(uint32)sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA)&&sWorld.getWUZHUConfig(WUZHU_MOB_HEAADD)<100.0f)
		{
			curhealth=uint32(sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA)+(curhealth-sWorld.getWUZHUConfig(WUZHU_MOB_MAXHEA))*sWorld.getWUZHUConfig(WUZHU_MOB_HEAADD)/100);
		}
		//wuzhu end
        if (curhealth < 1)
            curhealth = 1;
