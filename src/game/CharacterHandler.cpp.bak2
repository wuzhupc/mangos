...
bool LoginQueryHolder::Initialize()
{
    ...
    // !!! NOTE: including unused `zone`,`online`//wuzhu
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADFROM,            "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags,"
        ...
        "health, power1, power2, power3, power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, vip_integral FROM characters WHERE guid = '%u'", m_guid.GetCounter());//wuzhu
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    // Send MOTD
    {
		...
		//wuzhu start
		if(rand()%2==0)
			sWorld.SendWorldText(8211,pCurrChar->GetName());//wzuhu 发送欢迎信息
		else
			sWorld.SendWorldText(8212,pCurrChar->GetName());//wzuhu 发送欢迎信息
		//wuzhu end
