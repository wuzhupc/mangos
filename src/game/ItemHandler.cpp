...
#include "Chat.h"
#include "Spell.h"//wuzhu
#include "ScriptMgr.h"//wuzhu
...
void WorldSession::HandleReadItemOpcode(WorldPacket& recv_data)
{
    ...
    if (pItem && pItem->GetProto()->PageText)
    {
        ...
        SpellCastTargets targets;//wuzhu
		if(!sScriptMgr.OnItemUse(GetPlayer(),pItem,targets)){//wuzhu
        if (msg == EQUIP_ERR_OK)
        {
...
void WorldSession::HandleSellItemOpcode(WorldPacket& recv_data)
{
	...
	Item *pVipItem = _player->GetItemByGuid( vendorGuid );//wuzhu 宝石商店 //主函数是处理卖物品的
	if(!pVipItem){//wuzhu
    if (!pCreature)
    {
...
void WorldSession::HandleBuybackItem(WorldPacket& recv_data)
{
    ...
    Creature* pCreature = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
	Item *pVipItem = _player->GetItemByGuid( vendorGuid );//wuzhu 宝石商店//主函数是处理买物品的
	if(!pVipItem){//wuzhu
    if (!pCreature)
    {
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPacket& recv_data)
{
	...
	if(!pCreature)
	{ 
		GetPlayer()->BuyItemFromItemVendorSlot(vendorGuid, slot, item, count, bag, bagslot,vip_item_menu) ;
		return;
	}//wuzhu end
...
void WorldSession::HandleBuyItemOpcode(WorldPacket& recv_data)
{
	...
	if(!pCreature)
	{ 
		GetPlayer()->BuyItemFromItemVendorSlot(vendorGuid, slot, item, count, NULL_BAG, NULL_SLOT,vip_item_menu) ;
		return;
	}//wuzhu end
...
//wuzhu 宝石商店 start 参考WorldSession::SendListInventory 
void WorldSession::OpenShop(uint64 vguid, uint32 menu)
{
	//error_log("WUZHU WORLD: Sent SMSG_LIST_INVENTORY ---wuzhu" );

	ObjectGuid vendorguid=ObjectGuid(vguid);

	Item *p_Item = GetPlayer()->GetItemByGuid(vendorguid);	
	if (!p_Item)
	{
		//error_log( "WUZHU WORLD: OpenShop - %s not found or you can't interact with him.", vendorguid.GetString().c_str());
		_player->SendSellError( SELL_ERR_CANT_FIND_VENDOR, NULL, ObjectGuid(), 0);
		return;
	}
	vip_item_menu=menu;
	...
	p_Item->LoadVipGoods(menu);

	uint8 numitems = p_Item->GetVipItemCount();
	uint8 count = 0;
	...
	{	
		if( VipItem const* crItem = p_Item->GetVipItem(i) )
		{
			if( ItemPrototype const *pProto=ObjectMgr::GetItemPrototype(crItem->id))
			{
